...
window.location.href = `/bombparty?idplayer=${idplayer}&Username=${Username}&Loca=${Loca}`;
    }
    else
    if (GameMode==2)
    {
        ...
        window.location.href = `/DetectiveWord?idplayer=${idplayer}&Username=${Username}&Loca=${Loca}&Of=${Username}&range=${range}&gameMode=${GameMode}`;  
    }
    else
    if (GameMode==3)
    {
        var randomNumber = Math.floor(Math.random() * (9000)) + 1000;
        var idplayer=randomNumber;
        window.location.href = `/HarvestFestival?idplayer=${idplayer}&Username=${Username}&Loca=${Loca}&Of=${Username}&range=${range}&gameMode=${GameMode}`;  
    }
});
   ...
   alert("Vui lòng nhập mã code để tham gia phòng");
    ...
    {   
        ...
        {   check=true;
            ...
            window.location.href = `/HarvestFestival?idplayer=${idplayer}&Username=${Username}&Loca=${Loca}&Of=${Of}`;
            else
            if (rooms[i].gameMode==2)
            window.location.href = `/DetectiveWord?idplayer=${idplayer}&Username=${Username}&Loca=${Loca}&Of=${Of}`;
            break;
         }
        }
        if (check===false)
